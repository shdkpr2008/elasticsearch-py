...
def fetch_es_repo():
    ...
    repo_path = environ.get(
        "TEST_ES_REPO",
        abspath(join(dirname(__file__), pardir, pardir, "elasticsearch7")),
    )
    ...
    from test_elasticsearch7.test_server import get_client
    from test_elasticsearch7.test_cases import SkipTest
...
def run_all(argv=None):
    ...
    if argv is None:
        argv = [
            ...
            "--with-xunit",
            "--with-xcoverage",
            "--cover-package=elasticsearch7",
            "--cover-erase",
            "--logging-filter=elasticsearch7",
            "--logging-level=DEBUG",
            "--verbose",
            "--with-id",
        ]
